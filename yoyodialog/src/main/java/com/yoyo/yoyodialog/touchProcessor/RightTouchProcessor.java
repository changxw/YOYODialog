...
public class RightTouchProcessor extends TouchProcessor {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (hasContentTouched) {
                    ...
                    float slideSpeed = -deltaX * 1.0f / deltaTime;
                    if (deltaX < -currRect.width() / 2 || slideSpeed > 1.0f) {
                        smoothDismiss(dialog, dialog.getWindow().getDecorView().getWidth() - currRect.left, slope);
                    } else {
            ...
            case MotionEvent.ACTION_MOVE:
                if (hasContentTouched) {
                    ...
                    } else if (contentView.getLeft() + deltaX >= originalRect.left) {
                        ViewCompat.offsetLeftAndRight(contentView, (int) deltaX);
                    }

                    int distance = originalRect.width();
                    float scrollDistance = Math.max(contentView.getLeft() + deltaX - originalRect.left, 0);
