...
public class BottomTouchProcessor extends TouchProcessor {
            ...
            case MotionEvent.ACTION_UP:
                if (hasContentTouched) {
                    ...
                    if (deltaY < -currRect.height() / 2 || slideSpeed > 1.0f) {
                        smoothDismiss(dialog, dialog.getWindow().getDecorView().getHeight() - currRect.top, slope);
                    } else {
            ...
            case MotionEvent.ACTION_MOVE:
                if (hasContentTouched) {
                    ...
                    if (deltaY > 0) {
                        ...
                        isSliding = true;
                    } else if (contentView.getTop() + deltaY >= originalRect.top) {
                        ViewCompat.offsetTopAndBottom(contentView, (int) deltaY);
                        isSliding = true;
                    }
    ...
    private void smoothBack(YOYODialog dialog, int offsetY, final float slope) {
        final View contentView = dialog.getContentView();
        ValueAnimator valueAnimator = ValueAnimator.ofInt(0, (int) offsetY).setDuration(400);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            private int lastX = 0;
            private int lastY = 0;
